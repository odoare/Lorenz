...
void LorenzAudioProcessor::prepareToPlay (double sampleRate, int samplesPerBlock)
{
    ...
    pitchBufferSize = fftSize;
    bufferSize = samplesPerBlock;

    // The analysis buffer size should match the FFT size for the YIN algorithm.
...
void LorenzAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    float freq = getPitchYIN(analysisBuffer.getReadPointer(0), analysisBuffer.getNumSamples(), getSampleRate());
    if (freq > 0.0f)
    ...
    {
        // --- PID Controller ---
        // We only update the PID controller periodically, not on every audio block.
        // This prevents instability from rapid, noisy frequency measurements.
        ...
        // Update target dt
        float newDt = dtTarget + (dtProportional * Kp) + dtIntegral + (dtDerivative * Kd);
        dtTarget = timestepRangedParam->getNormalisableRange().snapToLegalValue(newDt); // Clamp to the parameter's full legal range

        // Convert the real-world value back to a normalized value (0.0 - 1.0)
        auto normalizedValue = timestepRangedParam->getNormalisableRange().convertTo0to1(newDt);
...
float LorenzAudioProcessor::getPitchYIN(const float* audioData, int dataSize, float sr)
{
    // dataSize must be equal to fftSize for this implementation.
    jassert (dataSize == fftSize);

    // Step 1: Autocorrelation via FFT
    // The forward FFT for real data is out-of-place.
    // We need to copy the input data to the first half of our FFT buffer.
    std::copy (audioData, audioData + dataSize, fftData.begin());
    // The transform is done in-place on fftData.
    forwardFFT.performRealOnlyForwardTransform (fftData.data(), true);

    // Compute power spectrum (magnitude squared)
    // The result is stored in a packed format. We iterate through the complex numbers.
    for (int i = 0; i < fftSize; i += 2)
    {
        const float real = fftData[i];
        const float imag = fftData[i + 1];
        fftData[i] = real * real + imag * imag; // Power for this bin
        fftData[i + 1] = 0.0f; // The imaginary part of the power spectrum is zero
    }

    // Perform inverse FFT to get autocorrelation. This is also out-of-place.
    forwardFFT.performRealOnlyInverseTransform (fftData.data());

    // Step 2: Difference function
    // The inverse transform result is now in fftData, we need to copy it to yinBuffer
    std::copy(fftData.begin(), fftData.begin() + (dataSize / 2), yinBuffer.begin());

    // The result in yinBuffer is now the autocorrelation function, r(tau).
    // We will now overwrite it with the cumulative mean normalized difference function.
    const float autocorrelation_r0 = yinBuffer[0];
    yinBuffer[0] = 1.0f; // d_t(0) is 1

    float runningSum = 0.0f;
    for (int tau = 1; tau < dataSize / 2; ++tau)
    {
        // Simplified difference function: d(tau) = r(0) - r(tau)
        const float difference = autocorrelation_r0 - yinBuffer[tau];

        // Cumulative mean normalized difference function
        runningSum += difference;
        if (runningSum > 1e-9) // Avoid division by zero
    ...
    const float threshold = 0.15f; // A common threshold for YIN. Can be tuned.
    int period = -1;

    // Start search from a minimum tau to avoid detecting very high frequencies.
    // e.g., for sr=44100, min_tau=20 -> max_freq = 2205 Hz.
    const int min_tau = 20;

    for (int tau = min_tau; tau < dataSize / 2; ++tau)
    {
        if (yinBuffer[tau] < threshold && yinBuffer[tau] < yinBuffer[tau - 1])
        {
            // Find the minimum in this dip
            // Make sure we don't go out of bounds with tau+1
            while (tau + 1 < dataSize / 2 && yinBuffer[tau + 1] <= yinBuffer[tau])
                tau++;
    ...
    // Step 4: Calculate frequency from the period
    if (period != -1)
    {
        // Check for array bounds before interpolation
        if (period < 1 || period + 1 >= dataSize / 2)
            return sr / (float)period;

        // Parabolic interpolation for higher accuracy.
        // Find the coordinates of the minimum and its neighbors.
        const float x1 = (float)period - 1;
        const float y1 = yinBuffer[period - 1];
        const float x2 = (float)period;
        const float y2 = yinBuffer[period];
        const float x3 = (float)period + 1;
        const float y3 = yinBuffer[period + 1];

        // Calculate the vertex of the parabola passing through these three points.
        const float a = (y1 + y3 - 2.0f * y2) / 2.0f;
        const float b = (y3 - y1) / 2.0f;

        // Avoid division by zero if the three points are collinear
        if (std::abs(a) < 1e-6)
            return sr / (float)period;
            
        const float interpolatedPeriod = x2 - b / (2.0f * a);
        return sr / interpolatedPeriod;
    }

    return -1.0f; // Return -1 if no pitch is detected
}
